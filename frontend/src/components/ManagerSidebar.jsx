...
import { FiHome, FiPackage, FiUsers, FiBarChart2 } from "react-icons/fi";
      ...
      <aside className="hidden lg:block w-64 xl:w-72 shrink-0 sticky top-4 self-start">
        <div className="bg-white/90 rounded-2xl border border-slate-200/80 shadow-sm p-3">
          ...
          <nav className="flex flex-col gap-2">
            <NavItem to="/manager/dashboard" icon={FiHome} label="Dashboard" />
            <NavItem
              to="/manager/inventory"
              icon={FiPackage}
              label="Inventory"
            />
            <NavItem to="/manager/cadets" icon={FiUsers} label="Cadets" />
            <NavItem to="/manager/reports" icon={FiBarChart2} label="Reports" />
          </nav>
        </div>
      </aside>
